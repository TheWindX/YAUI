...
namespace ns_YAUI
{
    class UIStraitLine : UIWidget
    {
        ...
        Brush mBrush;
        bool mHorizen;
        public UIStraitLine(int length, int lineWidth, uint fillcolor = 0xffffffff, bool horizon = true)
        {
            mHorizen = horizon;
            _length = length;
        ...
        public static XmlNodeList fromXML(XmlNode node, out UIWidget ui, UIWidget p)
        {
            ...
            ui = new UIStraitLine(length, lineWidth, color, horizon);
            ui.fromXML(node);
            ui.paresent = p;
